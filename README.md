...
- **Gemini が生成したチャプター要約・日次ダイジェストの本文**は、git にはコミットしません。
  `check.yml` / `daily.yml` は状態（`state/`以下のJSON）を **GitHub Actionsのキャッシュ**
  （リポジトリの外からは閲覧できない領域）に保存する方式にしており、public な
  コミット履歴やファイル一覧に要約内容が残ることはありません
